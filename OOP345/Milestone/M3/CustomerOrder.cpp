...
namespace sdds {
		...
		for (size_t i = 0; i < m_cntItem && !isDone; i++) {
			if (m_lstItem[i]->m_itemName == station.getItemName() && !(m_lstItem[i]->m_isFilled)) {
				if (station.getQuantity() >= 1) {
					...
					m_lstItem[i]->m_serialNo = station.getNextSerialNumber();
	...
	void CustomerOrder::display(std::ostream& os) const
	{
		...
		for (size_t i = 0; i < m_cntItem; i++) {
			os << "[" << setw(6) << setfill('0') << right << m_lstItem[i]->m_serialNo << "] " << left << setw(m_widthField + 2) << setfill(' ') << m_lstItem[i]->m_itemName << " - " << (m_lstItem[i]->m_isFilled ? "FILLED" : "TO BE FILLED") << endl;
		}
	}
}
